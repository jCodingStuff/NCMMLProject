...
class GridworldMultiAgentv2(Env):
    ...
    def step(self, action: int):
        ...
        for i, resource in enumerate(self.state_resources):
            ...
            extracted = False
            for agent in self.state_agent:
                if (agent[0] >= resource[0] - self.radius and agent[0] <= resource[0] + self.radius and
                    agent[1] >= resource[1] - self.radius and agent[1] <= resource[1] + self.radius):
                    nb_agents_radius += 1
                    # If the resource and agent have the same coordinates [x,y] ...
                    if np.all(resource == agent):
                        # ... add resource extraction reward to total reward and mark resource as extracted
            ...
            for civilian in self.state_civilians:
                if (civilian[0] >= resource[0] - self.radius and civilian[0] <= resource[0] + self.radius and
                    civilian[1] >= resource[1] - self.radius and civilian[1] <= resource[1] + self.radius):
    ...
    def render(self, mode='human'):
        ...
        for civilian in self.state_civilians:
            # Compute center
            center = (civilian[0]*self.cell_pixels + self.cell_pixels/2, civilian[1]*self.cell_pixels + self.cell_pixels/2)
    ...
    # Observation generation function
    def observe(self):
        return np.concatenate((self.state_resources.flatten(), self.state_agent.flatten(), self.state_civilians.flatten()))
